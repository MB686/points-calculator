...
function renderSavedFoods() {
  ...
  foods.forEach((food, index) => {
    ...
    const editBtn = document.createElement('button');
    editBtn.className = 'edit-food-btn icon-btn';
    editBtn.setAttribute('aria-label', 'Edit');
    editBtn.setAttribute('title', 'Edit');
    editBtn.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1 1 0 0 0 0-1.41l-2.34-2.34a1 1 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>';
    editBtn.onclick = () => editSavedFood(index);

    const deleteBtn = document.createElement('button');
    deleteBtn.className = 'delete-food-btn icon-btn';
    deleteBtn.setAttribute('aria-label', 'Delete');
    deleteBtn.setAttribute('title', 'Delete');
    deleteBtn.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M6 19a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>';
...
(function initSwipe() {
  ...
  document.addEventListener('touchmove', (e) => {
    ...
    if (!isHorizontalSwipe && Math.abs(dx) > Math.abs(dy) * 1.5 && Math.abs(dx) > 10) {
